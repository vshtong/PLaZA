...
def sign_challenge(user_id, private_key, challenge_hex, algo_variant):
    crypto = LatticeCrypto(algo_variant=algo_variant)
    if not private_key:
        print("Private key not found")
        return None
    try:
        load_private_key(user_id)
    except:
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python client.py <user_id> <file_private_key> <challenge_hex> <algorithm_variant>")
        sys.exit(1)
    user_id = sys.argv[1]
    priv_key = sys.argv[2]
    challenge_hex = sys.argv[3]
    algorithm_variant = sys.argv[4]
    signature = sign_challenge(user_id, priv_key, challenge_hex, algorithm_variant)
